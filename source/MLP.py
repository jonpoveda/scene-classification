...
class multi_layer_perceptron(object):
    def __init__(self, img_size = 32,batch_size = 16, dataset_dir = '/home/datasets/scenes/MIT_split', model_fname = 'my_first_mlp.h5'):
		#initialyze model
        ...
        if not os.path.exists(self.DATASET_DIR):
		  colorprint(Color.RED, 'ERROR: dataset directory '+self.DATASET_DIR+' do not exists!\n')
		  quit()
    ...
    def build_MLP_model(self):
		...
		self.model.add(Reshape((self.IMG_SIZE*self.IMG_SIZE*3,),input_shape=(self.IMG_SIZE, self.IMG_SIZE, 3),name='first'))
		...
		plot_model(self.model, to_file='modelMLP.png', show_shapes=True, show_layer_names=True)
    ...
    def train_MLP_model(self):
		...
		if os.path.exists(self.MODEL_FNAME):
		  colorprint(Color.YELLOW, 'WARNING: model file '+self.MODEL_FNAME+' exists and will be overwritten!\n')
		...
		train_generator = train_datagen.flow_from_directory(
				...
				target_size=(self.IMG_SIZE, self.IMG_SIZE),  # all images will be resized to IMG_SIZExIMG_SIZE
		...
		colorprint(Color.BLUE, 'Done!\n')
		colorprint(Color.BLUE, 'Saving the model into '+self.MODEL_FNAME+' \n')
		self.model.save_weights(self.MODEL_FNAME)  # always save your weights after training or during training
		colorprint(Color.BLUE, 'Done!\n')
    ...
    def load_MLP_model(self):
        ...
        if not os.path.exists(self.MODEL_FNAME):
            colorprint(Color.YELLOW, 'Error: model file '+self.MODEL_FNAME+' exists and will be overwritten!\n')
            quit()
            
        colorprint(Color.BLUE, 'Loading the model from '+self.MODEL_FNAME+' \n')
        self.model.load_weights(self.MODEL_FNAME)  # always save your weights after training or during training
        colorprint(Color.BLUE, 'Done!\n')
    ...
    def get_layer_output(self, layer = 'last', image_set ='test' ):
		...
		model_layer = Model(input=self.model.input, output=self.model.get_layer(layer).output)
    ...
    def plot_results(self):
        ...
        colorprint(Color.BLUE, 'Evaluator \nAccuracy: {} \nPrecision: {} \nRecall: {} \nFscore: {}'.
        ...
        plt.show()
        plt.savefig('cm.jpg')
        colorprint(Color.BLUE, 'Final accuracy: ' + str(evaluator.accuracy) + '\n')
